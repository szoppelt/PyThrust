...
This script runs the powertrain co-design optimization to find the optimal theoretical
motor parameters (Kv, resistance, current limit) for a hover thrust of 4.903 N.
It then loads the brushless motor database and finds the closest matching
real-world motors sorted by internal resistance and weight.
"""
...
def main():
    ...
    # 4. Search and Filter Real Motors
    min_kv = opt_kv * 0.85
    max_kv = opt_kv * 1.15
    ...
    # Sort candidates by winding resistance
    ...
    if not candidates:
        ...
        for idx, c in enumerate(candidates[:5]):
            ...
            # Convert resistance to mOhm for readability
